...
import easyDB, { getRandomId, Data, API, DBTypes } from "easy-db-core";

export type FileType = "collection" | "file" | "backup";

export type NodeAPI<T extends DBTypes> = API<T> & {
...
export default function easyDBNodeCore<T extends DBTypes>({
    cacheExpirationTime, backup, saveFiles, isFile, readFile, writeFile, unlinkFile, getFileNames
}: Configuration): NodeAPI<T> {

    // not ask is file every save collection
    let lastBackupActualName = "";
        ...
        ...easyDB({
            ...
            async saveCollection(name: string, data: Data) {
                ...
                if (backup !== false) {
                    ...
                    if (actualName !== lastBackupActualName) {
                        lastBackupActualName = actualName;

                        const backupName = `${actualName}.zip`;
